      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-6 sm:space-y-8 max-w-4xl mx-auto">
          {experienceData.map((exp, index) => (
            <div
              key={index}
              className="p-5 sm:p-9 rounded-2xl sm:rounded-3xl bg-slate-50 border border-slate-200/80 shadow-sm hover:shadow-md transition-all space-y-4 sm:space-y-5"
            >
              
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2.5 sm:gap-4 border-b border-slate-200 pb-4 sm:pb-5">
                <div className="space-y-1 text-left">
                  <h3 className="text-lg sm:text-2xl font-extrabold text-slate-900 leading-snug">
                    {exp.role}
                  </h3>
                  <div className="text-sm sm:text-base font-bold text-blue-600">
                    {exp.company} <span className="text-slate-500 font-normal text-xs sm:text-sm">— {exp.location}</span>
                  </div>
                </div>

                <div className="self-start flex items-center gap-1.5 text-xs font-mono font-bold text-slate-700 bg-white border border-slate-200 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full shadow-2xs">
